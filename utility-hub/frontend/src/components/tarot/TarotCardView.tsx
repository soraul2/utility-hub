...
const TarotCardView: React.FC<TarotCardViewProps> = ({
      ...
      const imageUrl = (card.imagePath && card.imagePath.startsWith('http'))
            ? card.imagePath
            : (card.imagePath ? `${import.meta.env.VITE_API_BASE_URL || `http://${window.location.hostname}:8080`}${card.imagePath}` : 'https://placehold.co/300x500/1e1e2e/FFF?text=No+Image');
